...
int main(int argc, char** argv)
{
	...
	//HTTP2包处理器
	xhHTTP2Packet = RfcComponents_Http2Server_InitEx(st_ServiceConfig.st_XMax.nThread);
	...
	XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("启动服务中,初始化HTTP2组包成功,IO线程个数:%d"), st_ServiceConfig.st_XMax.nThread);
	//启动心跳
	if (st_ServiceConfig.st_XTime.nTimeOut > 0)
	{
		...
		XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("启动服务中,初始化HTTP2心跳服务成功,时间:%d,次数:%d"), st_ServiceConfig.st_XTime.nTimeOut, st_ServiceConfig.st_XTime.nTimeCheck);
	}
	else
	{
	...
	XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("启动服务中,启动HTTP2网络服务器成功,HTTP2端口:%d,IO:%d"), st_ServiceConfig.nPort, st_ServiceConfig.st_XMax.nIOThread);
	...
	//HTTP任务池
	BaseLib_OperatorMemory_Malloc((XPPPMEM)&ppSt_ListHTTPParam, st_ServiceConfig.st_XMax.nThread, sizeof(THREADPOOL_PARAMENT));
	...
	XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("启动服务中,启动HTTP2线程池服务成功,启动个数:%d"), st_ServiceConfig.st_XMax.nThread);
