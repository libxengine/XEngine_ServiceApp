...
int main(int argc, char** argv)
{
	...
	if (st_ServiceConfig.nPort > 0)
	{
		...
		if (st_ServiceConfig.st_XTime.nTimeOut > 0)
		{
			...
			XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("启动服务中,初始化业务心跳服务成功,时间:%d,次数:%d"), st_ServiceConfig.st_XTime.nTimeOut, st_ServiceConfig.st_XTime.nTimeCheck);
		}
		else
		{
		...
		XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("启动服务中,启动业务网络服务器成功,业务端口:%d,网络IO线程个数:%d"), st_ServiceConfig.nPort, st_ServiceConfig.st_XMax.nIOThread);
		//绑定网络事件
		...
		XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _T("启动服务中,启动业务线程池服务成功,启动个数:%d"), st_ServiceConfig.st_XMax.nThread);
	}
	else
	{
